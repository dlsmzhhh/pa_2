                ...
                while True:
                    match convert_bytes_to_int(read_bytes(client_socket, 8)):
                        ...
                        case 1:
                            ...
                            file_data = bytes("", encoding='utf8')
                            data_encrypted = bytes("", encoding='utf-8')
                            while True:
                                chunk_size = convert_bytes_to_int(
                                    read_bytes(client_socket, 8)
                                )
                                if chunk_size == 10000:
                                    break
                                encrypted_subdata = read_bytes(client_socket, chunk_size)

                                data_encrypted += encrypted_subdata

                                subdata = private_key.decrypt(
                                    encrypted_subdata, # in bytes
