...
def main(args):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        while state:
            ...
            # Send the file
            data_encrypted = bytes("", encoding='utf-8')
            with open(filename, mode="rb") as fp:
                ...
                cur = 0
                total_len = len(data)
                while chunk*cur<total_len:
                    if chunk*(cur+1)<total_len:
                        subdata = data[cur*chunk:(cur+1)*chunk]
                    else:
                        subdata = data[cur*chunk:]
                    cur += 1

                    # encrypt with padding
                    subdata_encrypted = server_public_key.encrypt(
                        subdata,
                        padding.OAEP(
                            mgf=padding.MGF1(hashes.SHA256()),
                    ...
                    data_encrypted += subdata_encrypted
                    s.sendall(convert_int_to_bytes(len(subdata_encrypted)))
                    s.sendall(subdata_encrypted)
                s.sendall(convert_int_to_bytes(10000))
